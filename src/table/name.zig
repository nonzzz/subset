...
pub fn init(allocator: Allocator, byte_reader: *reader.ByteReader, parsed_tables: *ParsedTables) !Table {
    return NameTable.init(allocator, byte_reader, parsed_tables);
}
...
test "parse name table" {
    ...
    var table = try NameTable.init(allocator, &byte_reader, &dummy_parsed_tables);
    try table.parse();
    defer table.deinit();
    const name_table = table.cast(NameTable);
